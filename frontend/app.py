...
def _remove_bg_bytes(data: bytes, session, erode_px: int) -> bytes:
    try:
...
def _clean_alpha_edges_rgba(
    im_rgba, erode_px: int, feather_px: float, autocontrast: bool
):
...
def index():
    if request.method == "POST":
        ...
        if input_type == "image":
            ...
            model_rel = (
                ...
                if glb_path.exists()
                    ...
                    f"results/{item_id}/{obj_path.name}" if obj_path.exists() else None
                )
            )

            ref_img_rel = (
                f"results/{item_id}/{(cut_ref if cut_ref.exists() else raw_ref).name}"
            )
        ...
        # ====== ТЕКСТ -> 3 варианта ======
            ...
            total_ms_sum = 0.0
            ref_cover_rel = None
            for i, (seed, style_suffix) in enumerate(zip(seeds, styles), start=1):
                ...
                model_rel = (
                    ...
                    if glb_path.exists()
                        ...
                        f"results/{item_id}/{obj_path.name}"
                        if obj_path.exists()
                        else None
                    )
                )
                ...
                timings = {
                    ...
                    "prep_text": _fmt_ms(
                        prep_times["bg_remove_ms"] + prep_times["prep_ms"]
                    ),
                    "bg_remove_text": _fmt_ms(prep_times["bg_remove_ms"]),
                    "mesh_text": _fmt_ms(mesh_ms),
