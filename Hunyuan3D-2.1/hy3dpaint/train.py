...
if __name__ == "__main__":
    ...
    if noise_in_channels is not None:
        with torch.no_grad():
            ...
            new_conv_in.weight[:, : model_unet.conv_in.in_channels, :, :].copy_(
                model_unet.conv_in.weight
            )

            new_conv_in.bias.zero_()
            new_conv_in.bias[: model_unet.conv_in.bias.size(0)].copy_(
                model_unet.conv_in.bias
            )
    ...
    if hasattr(model.unet, "controlnet"):
        if config.model.params.get("control_in_channels", None):
            ...
            with torch.no_grad():
                new_condition_conv_in.weight[
                    :, : condition_conv_in.in_channels, :, :
                ] = condition_conv_in.weight
                if condition_conv_in.bias is not None:
                    new_condition_conv_in.bias = condition_conv_in.bias

            model.unet.controlnet.controlnet_cond_embedding.conv_in = (
                new_condition_conv_in
            )

    rank_zero_print("Loaded Init ...")
    ...
    if getattr(config, "resume_from", None):
        ...
        sd0 = {
            k[len(model_unet_prefix) :]: v
            for k, v in sds.items()
            if model_unet_prefix in k
        }
