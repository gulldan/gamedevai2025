...
def mipmap_linear_grid_put_2d(
    H, W, coords, values, min_resolution=128, return_count=False
):
    """
...
def _format_output(
    image: torch.Tensor, return_type: str
...
class MeshRender:
    ...
    def _create_view_state(self, config: RenderConfig) -> ViewState:
        ...
        r_mv = get_mv_matrix(
            ...
            if config.camera_distance is None
            else config.camera_distance,
            center=config.center,
        )

        pos_camera = transform_pos(r_mv, self.vtx_pos, keepdim=True)
        pos_clip = transform_pos(proj, pos_camera)
        resolution = _ensure_resolution_format(
            config.resolution, self.default_resolution
        )
    ...
    def _get_normals_for_shading(
        self, view_state: ViewState, use_abs_coor: bool = False
    ) -> torch.Tensor:
        ...
        rast_out, _ = self.raster_rasterize(
            view_state.pos_clip, self.pos_idx, resolution=view_state.resolution
        )
        ...
        if self.shader_type == "vertex":
            ...
            normal = torch.zeros(
                rast_out.shape[0], rast_out.shape[1], rast_out.shape[2], 3
            ).to(rast_out)
            normal.reshape(-1, 3)[tri_ids_mask.view(-1)] = face_normals.reshape(-1, 3)[
                tri_ids[tri_ids_mask].view(-1)
            ]

        return normal, rast_out

    def _unified_render_pipeline(
        self, config: RenderConfig, mode: RenderMode, **kwargs
    ) -> torch.Tensor:
        ...
        if mode == RenderMode.ALPHA:
            ...
            result = _apply_background_mask(
                normal, visible_mask, config.bg_color, self.device
            )
            ...
            if self.use_antialias:
                result = self.raster_antialias(
                    result, rast_out, view_state.pos_clip, self.pos_idx
                )
            ...
            result = _apply_background_mask(
                position, visible_mask, config.bg_color, self.device
            )
    ...
    def save_mesh(self, mesh_path, downsample=False):
        ...
        if downsample:
            texture_data = cv2.resize(
                texture_data, (texture_data.shape[1] // 2, texture_data.shape[0] // 2)
            )
            if texture_metallic is not None:
                texture_metallic = cv2.resize(
                    texture_metallic,
                    (texture_metallic.shape[1] // 2, texture_metallic.shape[0] // 2),
                )
            if texture_roughness is not None:
                texture_roughness = cv2.resize(
                    texture_roughness,
                    (texture_roughness.shape[1] // 2, texture_roughness.shape[0] // 2),
                )
            if texture_normal is not None:
                texture_normal = cv2.resize(
                    texture_normal,
                    (texture_normal.shape[1] // 2, texture_normal.shape[0] // 2),
                )
    ...
    def _set_texture_unified(
        ...
        converted_tex = _convert_texture_format(
            tex, self.texture_size, self.device, force_set
        )
    ...
    def extract_textiles(self):
        ...
        vtx_uv = torch.cat(
            (
                self.vtx_uv,
                torch.zeros_like(self.vtx_uv[:, 0:1]),
                torch.ones_like(self.vtx_uv[:, 0:1]),
            ),
            axis=1,
        )
        ...
        rast_out, rast_out_db = self.raster_rasterize(
            vtx_uv, self.uv_idx, resolution=self.texture_size
        )
        ...
        position_normal, _ = self.raster_interpolate(
            vertex_normals[None, ...], rast_out, self.pos_idx
        )
        ...
        tri_ids_mask = tri_ids > 0
        tri_ids = ((tri_ids - 1) * tri_ids_mask).long()
        position_normal.reshape(-1, 3)[tri_ids_mask.view(-1)] = face_normals.reshape(
            -1, 3
        )[tri_ids[tri_ids_mask].view(-1)]
        ...
        texture_indices.view(-1)[grid[:, 0] * self.texture_size[1] + grid[:, 1]] = (
            torch.arange(grid.shape[0]).to(device=self.device, dtype=torch.long)
        )
    ...
    def render_normal(
        ...
        config = RenderConfig(
            elev, azim, camera_distance, center, resolution, bg_color, return_type
        )
    ...
    def render_alpha(
        ...
        config = RenderConfig(
            elev, azim, camera_distance, center, resolution, return_type=return_type
        )
    ...
    def back_project(
        ...
        if method == "linear":
            ...
            cos_map_uv = torch.nn.functional.cosine_similarity(
                lookat, normal_map.view(-1, 3)
            )  # .abs()
            cos_map_uv = cos_map_uv.view(1, 1, normal_map.shape[0], normal_map.shape[1])
            cos_map_uv = torch.nn.functional.max_pool2d(
                cos_map_uv, kernel_size=3, stride=1, padding=1
            )
            cos_map_uv = cos_map_uv.reshape(
            ...
            img_x = torch.clamp(
                ((v_proj[:, 0].clamp(-1, 1) * 0.5 + 0.5) * (resolution[0])).long(),
                0,
                resolution[0] - 1,
            )
            img_y = torch.clamp(
                ((v_proj[:, 1].clamp(-1, 1) * 0.5 + 0.5) * (resolution[1])).long(),
                0,
                resolution[1] - 1,
            )
            ...
            valid_idx = torch.where(
                (torch.abs(v_z - sampled_z) < depth_thres) & (sampled_m * sampled_w > 0)
            )[0]
            ...
            wx = ((v_proj[:, 0] * 0.5 + 0.5) * resolution[0] - img_x)[
                valid_idx
            ].reshape(-1, 1)
            wy = ((v_proj[:, 1] * 0.5 + 0.5) * resolution[1] - img_y)[
                valid_idx
            ].reshape(-1, 1)
            ...
            sampled_rgb = (rgb[indices] * (1 - wx) + rgb[indices_lr] * wx) * (
                1 - wy
            ) + (rgb[indices_rl] * (1 - wx) + rgb[indices_rr] * wx) * wy

            # return sampled_rgb, sampled_w, sampled_b, valid_idx
            texture = torch.zeros(
                self.texture_size[0], self.texture_size[1], channel, device=self.device
            ).reshape(-1, channel)
            cos_map = torch.zeros(
                self.texture_size[0], self.texture_size[1], 1, device=self.device
            ).reshape(-1)
            boundary_map = torch.zeros(
                self.texture_size[0], self.texture_size[1], 1, device=self.device
            ).reshape(-1)

            valid_tex_indices = (
                self.tex_grid[valid_idx, 0] * self.texture_size[1]
    ...
    def bake_texture(
        ...
        if isinstance(colors, torch.Tensor):
            colors = [
                colors[i, ...].float().permute(1, 2, 0) for i in range(colors.shape[0])
            ]
        else:
    ...
    @torch.no_grad()
    def uv_inpaint(
        self, texture, mask, vertex_inpaint=True, method="NS", return_float=False
    ):
        """
