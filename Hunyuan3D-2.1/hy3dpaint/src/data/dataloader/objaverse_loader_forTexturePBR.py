...
class TextureDataset(BaseDataset):
    def __init__(
        ...
        num_view=6,
        image_size=512,
        lighting_suffix_pool=["light_PL", "light_AL", "light_ENVMAP"],
    ):
        self.data = list()
    ...
    def __getitem__(self, index):
        ...
        if not cond_images:
            raise ValueError(
                f"No condition images found in {os.path.join(dirx, 'render_cond')}"
            )
        ...
        for i, image_gen in enumerate(images_gen):
            images_albedo.append(
                self.augment_image(self.load_image(image_gen, bg_gray)[0], bg_gray)
            )
            images_mr.append(
                self.augment_image(
                    self.load_image(image_gen.replace("_albedo", "_mr"), bg_gray)[0],
                    bg_gray,
                )
            )
            images_normal.append(
                self.augment_image(
                    self.load_image(image_gen.replace("_albedo", "_normal"), bg_gray)[
                        0
                    ],
                    bg_gray,
                )
            ...
            images_position.append(
                self.augment_image(
                    self.load_image(image_gen.replace("_albedo", "_pos"), bg_gray)[0],
                    bg_gray,
                )
            )
