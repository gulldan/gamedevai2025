...
class HunyuanPaintPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **cached_condition,
        ...
        images_vae = [
            image_vae.unsqueeze(0).permute(0, 3, 1, 2).unsqueeze(0)
            for image_vae in images_vae
        ]
        ...
        if "images_normal" in cached_condition:
            if isinstance(cached_condition["images_normal"], List):
                cached_condition["images_normal"] = convert_pil_list_to_tensor(
                    cached_condition["images_normal"]
                )

            cached_condition["embeds_normal"] = self.encode_images(
                cached_condition["images_normal"]
            )

        if "images_position" in cached_condition:
            if isinstance(cached_condition["images_position"], List):
                cached_condition["images_position"] = convert_pil_list_to_tensor(
                    cached_condition["images_position"]
                )

            cached_condition["position_maps"] = cached_condition["images_position"]
            cached_condition["embeds_position"] = self.encode_images(
                cached_condition["images_position"]
            )
        ...
        if guidance_scale > 1:
            if self.unet.use_ra:
                cached_condition["ref_latents"] = cached_condition[
                    "ref_latents"
                ].repeat(3, *([1] * (cached_condition["ref_latents"].dim() - 1)))
                cached_condition["ref_scale"] = torch.as_tensor([0.0, 1.0, 1.0]).to(
            ...
            if "embeds_normal" in cached_condition:
                cached_condition["embeds_normal"] = cached_condition[
                    "embeds_normal"
                ].repeat(3, *([1] * (cached_condition["embeds_normal"].dim() - 1)))

            if "embeds_position" in cached_condition:
                cached_condition["embeds_position"] = cached_condition[
                    "embeds_position"
                ].repeat(3, *([1] * (cached_condition["embeds_position"].dim() - 1)))

            if "position_maps" in cached_condition:
                cached_condition["position_maps"] = cached_condition[
                    "position_maps"
                ].repeat(3, *([1] * (cached_condition["position_maps"].dim() - 1)))
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = (
                    latents.repeat(3, 1, 1, 1, 1, 1)
                    if self.do_classifier_free_guidance
                    else latents
                )
                latent_model_input = rearrange(
                    latent_model_input, "b n_pbr n c h w -> (b n_pbr n) c h w"
                )
                ...
                if self.do_classifier_free_guidance:
                    ...
                    noise_pred_uncond, noise_pred_ref, noise_pred_full = (
                        noise_pred.chunk(3)
                    )
                    ...
                    view_scale_tensor = (
                        torch.from_numpy(
                            np.asarray([cam_mapping(azim) for azim in camera_azims])
                        )
                        .unsqueeze(0)
                        .repeat(n_pbr, 1)
                        .view(-1)
                    ...
                    noise_pred += (
                        self.guidance_scale
                        * view_scale_tensor
                        * (noise_pred_full - noise_pred_ref)
                    )
