    ...
    def forward(self, images):
        ...
        if isinstance(images, torch.Tensor):
            ...
            dino_proceesed_images = torch.stack(
                [torch.from_numpy(np.array(image)) for image in dino_proceesed_images],
                dim=0,
            )
        ...
        dino_hidden_states = rearrange(
            dino_hidden_states.to(dino_param), "(b n) l c -> b (n l) c", b=batch_size
        )
...
def compute_voxel_grid_mask(position, grid_resolution=8):
    ...
    grid_position_expanded_1 = grid_position.unsqueeze(2).unsqueeze(
        4
    )  # 形状变为 B, N, 1, L, 1, 3
    grid_position_expanded_2 = grid_position.unsqueeze(1).unsqueeze(
        3
    )  # 形状变为 B, 1, N, 1, L, 3

    # 计算欧氏距离
    distances = torch.norm(
        grid_position_expanded_1 - grid_position_expanded_2, dim=-1
    )  # 形状为 B, N, N, L, L
    ...
    def _initialize_attn_weights(self):
        ...
        if self.use_mda:
            for token in self.pbr_setting:
                if token == "albedo":
                    continue
                getattr(self.attn1.processor, f"to_q_{token}").load_state_dict(
                    self.attn1.to_q.state_dict()
                )
                getattr(self.attn1.processor, f"to_k_{token}").load_state_dict(
                    self.attn1.to_k.state_dict()
                )
                getattr(self.attn1.processor, f"to_v_{token}").load_state_dict(
                    self.attn1.to_v.state_dict()
                )
                getattr(self.attn1.processor, f"to_out_{token}").load_state_dict(
                    self.attn1.to_out.state_dict()
                )
            ...
            with torch.no_grad():
                ...
                for token in self.pbr_setting:
                    ...
                    for layer in getattr(
                        self.attn_refview.processor, f"to_out_{token}"
                    ):
                        for param in layer.parameters():
                            param.zero_()
    ...
    def forward(
        ...
        if "r" in mode and self.use_ra:
            ...
            if isinstance(ref_scale, torch.Tensor):
                ref_scale_timing = (
                    ref_scale.unsqueeze(1).repeat(1, num_in_batch * N_pbr).view(-1)
                )
        ...
        if self.attn2 is not None:
            ...
            # dino attn
            if self.use_dino:
                dino_hidden_states = dino_hidden_states.unsqueeze(1).repeat(
                    1, N_pbr * num_in_batch, 1, 1
                )
                dino_hidden_states = rearrange(
                    dino_hidden_states, "b n l c -> (b n) l c"
                )
...
class ImageProjModel(torch.nn.Module):
    ...
    def forward(self, image_embeds):
        ...
        clip_extra_context_tokens = rearrange(
            clip_extra_context_tokens, "(b nt) n c -> b (nt n) c", nt=num_token
        )
        ...
        if self.use_learned_text_clip:
            for token in self.pbr_setting:
                self.unet.register_parameter(
                    ...
                    nn.Parameter(torch.zeros(self.pbr_token_channels, 1024)),
                )
            self.unet.learned_text_clip_ref = nn.Parameter(
                torch.zeros(self.pbr_token_channels, 1024)
            )
    ...
    def forward(
        ...
        **cached_condition,
        ...
        if "embeds_normal" in cached_condition:
            sample.append(
                cached_condition["embeds_normal"]
                .unsqueeze(1)
                .repeat(1, N_pbr, 1, 1, 1, 1)
            )
        if "embeds_position" in cached_condition:
            sample.append(
                cached_condition["embeds_position"]
                .unsqueeze(1)
                .repeat(1, N_pbr, 1, 1, 1, 1)
            )
        ...
        encoder_hidden_states_gen = encoder_hidden_states.unsqueeze(-3).repeat(
            1, 1, N_gen, 1, 1
        )
        encoder_hidden_states_gen = rearrange(
            encoder_hidden_states_gen, "b n_pbr n l c -> (b n_pbr n) l c"
        )

        if added_cond_kwargs is not None:
            text_embeds_gen = (
                added_cond_kwargs["text_embeds"].unsqueeze(1).repeat(1, N_gen, 1)
            )
            text_embeds_gen = rearrange(text_embeds_gen, "b n c -> (b n) c")
            time_ids_gen = (
                added_cond_kwargs["time_ids"].unsqueeze(1).repeat(1, N_gen, 1)
            )
            time_ids_gen = rearrange(time_ids_gen, "b n c -> (b n) c")
            added_cond_kwargs_gen = {
                "text_embeds": text_embeds_gen,
                "time_ids": time_ids_gen,
            }
        else:
        ...
        if self.use_position_rope:
            if "position_voxel_indices" in cached_condition["cache"]:
                position_voxel_indices = cached_condition["cache"][
                ...
                if "position_maps" in cached_condition:
                    ...
                    cached_condition["cache"]["position_voxel_indices"] = (
                        position_voxel_indices
                    )
        else:
            position_voxel_indices = None
        ...
        if self.use_dino:
            if "dino_hidden_states_proj" in cached_condition["cache"]:
                dino_hidden_states = cached_condition["cache"][
                ...
                cached_condition["cache"]["dino_hidden_states_proj"] = (
                    dino_hidden_states
                )
        else:
        ...
        if self.use_ra:
            if "condition_embed_dict" in cached_condition["cache"]:
                ...
                encoder_hidden_states_ref = self.unet.learned_text_clip_ref.repeat(
                    B, N_ref, 1, 1
                )
                ...
                if added_cond_kwargs is not None:
                    ...
                    time_ids_ref = (
                        added_cond_kwargs["time_ids"].unsqueeze(1).repeat(1, N_ref, 1)
                    )
        ...
        return self.unet(
            ...
            *args,
            ...
            down_intrablock_additional_residuals=(
                [
                    sample.to(dtype=self.unet.dtype)
                    for sample in down_intrablock_additional_residuals
                ]
