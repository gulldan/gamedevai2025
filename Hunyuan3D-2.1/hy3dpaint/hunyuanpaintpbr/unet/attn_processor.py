...
class AttnUtils:
    ...
    def prepare_hidden_states(
        ...
        attn,
        temb,
        spatial_norm_attr="spatial_norm",
        group_norm_attr="group_norm",
    ):
        """
            ...
            for module_type in module_types:
                if module_type == "qkv":
                    self.register_module(
                        f"to_q_{pbr_token}",
                        nn.Linear(self.query_dim, self.inner_dim, bias=self.use_bias),
                    )
                    self.register_module(
                        f"to_k_{pbr_token}",
                        nn.Linear(
                            self.cross_attention_dim, self.inner_dim, bias=self.use_bias
                        ),
                    )
                    self.register_module(
                        f"to_v_{pbr_token}",
...
class RotaryEmbedding:
    ...
    def get_1d_rotary_pos_embed(
        ...
        freqs = (
            ...
            / (
                theta
                ** (
                    torch.arange(0, dim, 2, dtype=pos.dtype, device=pos.device)[
                        : (dim // 2)
                    ]
                    / dim
                )
    ...
    def get_3d_rotary_pos_embed(
        position, embed_dim, voxel_resolution, theta: int = 10000
    ):
        """
        ...
        grid = torch.arange(
            voxel_resolution, dtype=torch.float32, device=position.device
        )
...
class AttnCore:
    ...
    def process_attention_base(
        ...
        **kwargs,
        ...
        hidden_states, residual, input_ndim, shape_info = (
            AttnUtils.prepare_hidden_states(hidden_states, attn, temb)
        )
        ...
        attention_mask = AttnUtils.prepare_attention_mask(
            attention_mask, attn, sequence_length, batch_size
        )
        ...
        query = AttnUtils.reshape_qkv_for_attention(
            query, batch_size, attn.heads, head_dim
        )
        key = AttnUtils.reshape_qkv_for_attention(key, batch_size, attn.heads, head_dim)
        value = AttnUtils.reshape_qkv_for_attention(
            value, batch_size, attn.heads, value.shape[-1] // attn.heads
        )

        # Apply normalization
        query, key = AttnUtils.apply_norms(
            query, key, getattr(attn, "norm_q", None), getattr(attn, "norm_k", None)
        )
...
class PoseRoPEAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
            ...
            if position_indices is not None:
                if head_dim in position_indices:
                    ...
                    image_rotary_emb = RotaryEmbedding.get_3d_rotary_pos_embed(
                        rearrange(
                            position_indices["voxel_indices"]
                            .unsqueeze(1)
                            .repeat(1, n_pbrs, 1, 1),
        ...
        hidden_states, residual, input_ndim, shape_info, batch_size, heads, head_dim = (
            AttnCore.process_attention_base(
                attn,
                hidden_states,
                encoder_hidden_states,
                attention_mask,
        ...
        return AttnUtils.finalize_output(
            hidden_states, input_ndim, shape_info, attn, residual, attn.to_out
        )
...
class SelfAttnProcessor2_0(BaseAttnProcessor):
    ...
    def process_single(
        ...
        **kwargs,
        ...
        if multiple_devices:
            device = (
                torch.device("cuda:0") if token == "albedo" else torch.device("cuda:1")
            )
            for attr in [
                f"to_q{token_suffix}",
                f"to_k{token_suffix}",
                f"to_v{token_suffix}",
                f"to_out{token_suffix}",
            ]:
        ...
        return AttnUtils.finalize_output(
            ...
            attn,
            residual,
            getattr(target, f"to_out{token_suffix}"),
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for token, pbr_hs in zip(self.pbr_setting, pbr_hidden_states):
            processed_hs = rearrange(pbr_hs, "b n_pbrs n l c -> (b n_pbrs n) l c").to(
                "cuda:0"
            )
            result = self.process_single(
                attn, processed_hs, None, attention_mask, temb, token, False
            )
            results.append(result)

        outputs = [
            rearrange(result, "(b n_pbrs n) l c -> b n_pbrs n l c", b=B, n_pbrs=1)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        def get_qkv(attn, hidden_states, encoder_hidden_states, **kwargs):
            ...
            for token in [
                "_" + token for token in self.pbr_settings if token != "albedo"
            ]:
                value_list.append(
                    getattr(attn.processor, f"to_v{token}")(encoder_hidden_states)
                )
        ...
        for i, hs in enumerate(hidden_states_list):
            hs = (
                hs.transpose(1, 2)
                .reshape(batch_size, -1, heads * head_dim)
                .to(hs.dtype)
            )
            token_suffix = (
                "_" + self.pbr_settings[i] if self.pbr_settings[i] != "albedo" else ""
            )
