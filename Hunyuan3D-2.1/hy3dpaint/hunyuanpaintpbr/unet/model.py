...
class HunyuanPaint(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        if control_net_config is not None:
            pipeline.unet = pipeline.unet.bfloat16().requires_grad_(
                control_net_config.train_unet
            )
    ...
    def prepare_batch_data(self, batch):
        ...
        images_cond = batch["images_cond"].to(
            self.device
        )  # (B, M, C, H, W), where M is the number of reference images
        cond_imgs, cond_imgs_another = (
            images_cond[:, 0:1, ...],
            images_cond[:, 1:2, ...],
        )
        ...
        cond_imgs_another = v2.functional.resize(
            cond_imgs_another, cond_size, interpolation=3, antialias=True
        ).clamp(0, 1)
        ...
        if "images_normal" in batch:
            ...
            images_normal = v2.functional.resize(
                images_normal, self.view_size, interpolation=3, antialias=True
            ).clamp(0, 1)
    ...
    def encode_images(self, images):
        ...
        latents = (
            ...
            if image_ndims == 5
                ...
                latents, "(b n_pbrs n) c h w -> b n_pbrs n c h w", b=B, n_pbrs=N_pbrs
            )
        )
    ...
    def forward_unet(self, latents, t, **cached_condition):
        ...
        pred_noise = self.pipeline.unet(
            latents, t, encoder_hidden_states=shading_embeds, **cached_condition
        )
        return pred_noise[0]
    ...
    def training_step(self, batch, batch_idx):
        ...
        cond_imgs, cond_imgs_another, target_imgs, normal_imgs, position_imgs = (
        ...
        for token in self.pbr_settings:
            if token in ["albedo", "mr"]:
                all_shading_tokens.append(
                    getattr(self.unet, f"learned_text_clip_{token}")
                    .unsqueeze(dim=0)
                    .repeat(B, 1, 1)
                )
        ...
        latents_noisy = self.train_scheduler.add_noise(gen_latents, noise, t).to(
            self.device
        )
        latents_noisy = rearrange(
            latents_noisy, "(b n_pbrs n) c h w -> b n_pbrs n c h w", b=B, n_pbrs=N_pbrs
        )
        ...
        for b in range(B):
            ...
            prob = np.random.rand()
            if prob < self.drop_cond_prob:
                dino_hidden_states[b, ...] = torch.zeros_like(
                    dino_hidden_states[b, ...]
                )
            prob = np.random.rand()
            if prob < self.drop_cond_prob:
                dino_hidden_states_another[b, ...] = torch.zeros_like(
                    dino_hidden_states_another[b, ...]
                )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        cond_imgs_tensor, _, target_imgs, normal_imgs, position_imgs = (
            self.prepare_batch_data(batch)
        )
        ...
        for i in range(cond_imgs_tensor.shape[0]):
            ...
            for j in range(cond_imgs_tensor.shape[1]):
                image_pils[-1].append(
                    v2.functional.to_pil_image(cond_imgs_tensor[i, j, ...])
                )
        ...
        for idx in range(len(image_pils)):
            ...
            cached_condition = dict(
                num_in_batch=self.num_view, N_pbrs=len(self.pbr_settings)
            )
            if normal_imgs is not None:
                cached_condition["images_normal"] = normal_imgs[0][idx, ...].unsqueeze(
                    0
                )
            if position_imgs is not None:
                cached_condition["images_position"] = position_imgs[0][
                    idx, ...
                ].unsqueeze(0)
            ...
            image = torch.cat(
                (torch.ones_like(image[:, :, :1, ...]) * 0.5, image), dim=2
            )
        ...
        all_target_pbrs = torch.cat(
            (
                cond_imgs_tensor.unsqueeze(1).repeat(
                    1, len(self.pbr_settings), 1, 1, 1, 1
                ),
                all_target_pbrs,
            ),
            dim=2,
        )
        all_target_pbrs = rearrange(
            all_target_pbrs, "b n_pbrs n c h w -> b c (n_pbrs h) (n w)"
        )
        gts = all_target_pbrs
    ...
    def configure_optimizers(self):
        ...
        def lr_lambda(step):
            ...
            if normalized_step < warm_up_step:
                lr_step = min_lr + (normalized_step / warm_up_step) * (
                    current_max_lr - min_lr
                )
            else:
                step_wc_wp = normalized_step - warm_up_step
                ratio = step_wc_wp / T_step
                lr_step = min_lr + 0.5 * (current_max_lr - min_lr) * (
