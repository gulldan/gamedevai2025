...
class ResampledShards(torch.utils.data.dataset.IterableDataset):
    def __init__(
        self, datalist, nshards=sys.maxsize, worker_seed=None, deterministic=False
    ):
        super().__init__()
...
class AlignedShapeLatentDataset(torch.utils.data.dataset.IterableDataset):
    ...
    def load_surface_sdf_points(self, rng, random_surface, sharpedge_surface):
        ...
        if self.pc_size > 0:
            ...
            if self.sharpedge_label:
                ...
                random_surface = np.concatenate(
                    (random_surface, sharpedge_label), axis=1
                )
            surface_normal.append(random_surface)

        if self.pc_sharpedge_size > 0:
            ind_sharpedge = rng.choice(
                sharpedge_surface.shape[0], self.pc_sharpedge_size, replace=False
            )
    ...
    def load_render(self, imgs_path):
        ...
        for image_path in imgs_choice:
            ...
            if self.padding:
                ...
                image, mask = padding(
                    image[
                        max(x_min - 5, 0) : min(x_max + 5, h),
                        max(y_min - 5, 0) : min(y_max + 5, w),
                    ],
                    mask[
                        max(x_min - 5, 0) : min(x_max + 5, h),
                        max(y_min - 5, 0) : min(y_max + 5, w),
    ...
    def decode(self, item):
        uid = item.split("/")[-1]
        render_img_paths = [
            os.path.join(item, f"render_cond/{i:03d}.png") for i in range(24)
        ]
    ...
    def transform(self, sample):
        ...
        surface, geo_points = self.load_surface_sdf_points(
            rng, random_surface, sharpedge_surface
        )
        sample = {
            "surface": surface,
    ...
    def __iter__(self):
        ...
        for data in ResampledShards(self.data_list):
            total_num += 1
            if total_num % 1000 == 0:
                print("Current failure rate of data loading:")
