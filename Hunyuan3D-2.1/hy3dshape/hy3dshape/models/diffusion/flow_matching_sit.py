...
class Diffuser(pl.LightningModule):
        ...
        *,
        ...
        if "transport" in scheduler_cfg:
            ...
            self.sampler = instantiate_from_config(
                scheduler_cfg.sampler, transport=self.transport
            )
    ...
    def ema_scope(self, context=None):
        ...
        try:
            ...
            if self.ema_config is not None and self.ema_config.get(
                "ema_inference", False
            ):
    ...
    def configure_optimizers(self) -> Tuple[List, List]:
        ...
        if self.optimizer_cfg.get("train_image_encoder", False):
            ...
            image_encoder_parameters_decay = [
                param for param in image_encoder_parameters_decay if param.requires_grad
            ]
            image_encoder_parameters_nodecay = [
                param
                for param in image_encoder_parameters_nodecay
                if param.requires_grad
            ]

            print(
                f"Image Encoder Params: {len(image_encoder_parameters_decay)} decay, "
            )
            print(
                f"Image Encoder Params: {len(image_encoder_parameters_nodecay)} nodecay, "
            )

            image_encoder_lr = self.optimizer_cfg["image_encoder_lr"]
            image_encoder_lr_multiply = self.optimizer_cfg.get(
                "image_encoder_lr_multiply", 1.0
            )
            image_encoder_lr = (
                image_encoder_lr
                if image_encoder_lr is not None
                else lr * image_encoder_lr_multiply
            )
            params_list.append(
                {
                    "params": image_encoder_parameters_decay,
                    "lr": image_encoder_lr,
                    "weight_decay": 0.05,
                }
            )
            params_list.append(
                {
                    "params": image_encoder_parameters_nodecay,
                    "lr": image_encoder_lr,
                    "weight_decay": 0.0,
                }
            )
        ...
        optimizer = instantiate_from_config(
            self.optimizer_cfg.optimizer, params=params_list, lr=lr
        )
    ...
    def forward(self, batch):
        with torch.autocast(
            ...
            contexts = self.cond_stage_model(
                image=batch.get("image"), text=batch.get("text"), mask=batch.get("mask")
            )

        with torch.autocast(device_type="cuda", dtype=torch.float16):
            with torch.no_grad():
                latents = self.first_stage_model.encode(
                    batch[self.first_stage_key], sample_posterior=True
                )
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            loss = self.transport.training_losses(
                self.model, latents, dict(contexts=contexts)
            )["loss"].mean()
        return loss
            ...
            with torch.amp.autocast(device_type="cuda"):
                try:
                    ...
                    outputs = self.pipeline(
                        image=image, mask=mask, generator=generator, **additional_params
                    )
