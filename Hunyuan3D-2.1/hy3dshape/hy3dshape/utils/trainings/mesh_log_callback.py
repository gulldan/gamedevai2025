...
class ImageConditionalASLDiffuserLogger(Callback):
    ...
    def log_local(
        ...
        folder = "gs-{:010}_e-{:06}_b-{:06}".format(
        ...
        for i in range(num_samples):
            ...
            if multi_views is not None:
                ...
                views_tag = html_util.to_image_embed_tag(
                    self.denormalize_image(multi_views_i)
                )
                table_tag = f"""
                <table border = "1">
                    <caption> {shape_tag_i} - {key_i} </caption>
    ...
    def log_sample(
        ...
        with torch.no_grad():
            ...
            outputs = pl_module.sample(batch=sample_batch, output_type="latents2mesh")
    ...
    def make_grid(self, images):  # return (3,h,w) in (0,1) ...
        ...
        image = torchvision.utils.make_grid(
            images_resized, nrow=2, padding=5, pad_value=255
        )
    ...
    def on_train_batch_end(
        ...
        if (
            ...
            and hasattr(pl_module, "sample")
            and callable(pl_module.sample)
    ...
    def on_train_batch_end(
        ...
        if pl_module.global_step % self.step_freq == 0:
            ...
            visual_dir = os.path.join(
                pl_module.logger.save_dir, self.save_dir, folder, folder_name
            )
            os.makedirs(visual_dir, exist_ok=True)
            ...
            if pl_module.global_rank == trainer.world_size - 1:
                ...
                image_paths = image_paths[
                    pl_module.global_rank * chunk_size : (pl_module.global_rank + 1)
                    * chunk_size
                ]

            print(
                f"Rank{pl_module.global_rank}: processing {len(image_paths)}|{len(self.file_list)} images"
            )
            for image_path in image_paths:
                ...
                with torch.no_grad():
                    mesh = pl_module.sample(batch={"image": image_path}, **self.kwargs)[
                        0
                    ][0]
