...
def load_surface_sharpegde(
    ...
    mesh = trimesh.Trimesh(
        vertices=original_vertices, faces=original_faces[:origin_num]
    )
    ...
    if num_fill == 0:
        ...
        random_surface_fill, random_normal_fill = sample_pointcloud(
            mesh_fill, num=num_fill
        )
        ...
        if isinstance(mesh, trimesh.scene.Scene):
            for idx, obj in enumerate(mesh.geometry.values()):
                if idx == 0:
                    ...
                    temp_mesh = temp_mesh + obj
            mesh = temp_mesh
        surface, mesh = load_surface_sharpegde(
            mesh, num_points=num_uniform_points, num_sharp_points=num_sharp_points
        )
        return surface
