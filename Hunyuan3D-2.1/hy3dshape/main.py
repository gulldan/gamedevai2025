...
def get_args():
    ...
    parser.add_argument(
        "--output_dir", type=str, help="the output directory to save everything."
    )
    parser.add_argument(
        "--ckpt_path", type=str, default="", help="the restore checkpoints."
    )
...
if __name__ == "__main__":
    ...
    if args.scale_lr:
        model.learning_rate = (
            accumulate_grad_batches * nodes * ngpus * batch_size * base_lr
        )
    ...
    if training_cfg.use_amp:
        ...
        amp_type = 32
        rank_zero_info("Using 32 bit precision")
    rank_zero_info("*" * 100)
