...
def sample_sdf(mesh, random_surface, sharp_surface):
    ...
    random_near_points = np.concatenate(
        [random_surface + offset1, random_surface + offset2], axis=0
    )
    ...
    sharp_near_points = np.concatenate(
        [
            ...
            + np.random.normal(scale=0.06, size=(unit_num, 3)),
            ...
            + np.random.normal(scale=0.04, size=(len(sharp_surface) - 5 * unit_num, 3)),
        ],
        axis=0,
    )
...
def Watertight(V, F, epsilon=2.0 / 256, grid_res=256):
    ...
    mc_verts, mc_faces = igl.marching_cubes(
        epsilon - np.abs(sdf), grid_points, grid_res, grid_res, grid_res, 0.0
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process an OBJ file and output surface and SDF data."
    )
